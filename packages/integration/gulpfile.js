...
const config = {
  ...
  less      : ['src/**/*.less'],
  dist      : argv.out || './dist/',
  js        : 'hawtio-online-integration.js',
  dts       : 'hawtio-online-integration.d.ts',
  css       : 'hawtio-online-integration.css',
};
...
// Helpers
const task = (name, fn) => {
  fn.displayName = `[integration] ${name}`; return fn;
};
...
function compileTemplates() {
  ...
  .pipe(plugins.angularTemplatecache({
    ...
    root          : 'src/',
    standalone    : true,
    module        : 'hawtio-online-integration-templates',
    templateFooter: '}]); hawtioPluginLoader.addModule("hawtio-online-integration-templates");',
  }))
  .pipe(gulp.dest('.', { cwd: __dirname }));
}
